...
struct State {
  State(int end, int phrase_len, const vector<int>& phrase,
        const vector<int>& subpatterns_start, shared_ptr<TrieNode> node,
        bool starts_with_x) :
      end(end), phrase_len(phrase_len), phrase(phrase),
...
Grammar GrammarExtractor::GetGrammar(
  ...
  for (size_t i = 0; i < lattice.GetSize(); ++i) {
    states.push(State(i, 0, vector<int>(), vector<int>(1, i), root, false));
  }
  for (size_t i = min_gap_size; i < lattice.GetSize(); ++i) {
    states.push(State(i, min_gap_size, vector<int>(1, first_x),
  ...
  while (!states.empty()) {
    ...
    for (const auto& edge: lattice.GetTransitions(state.end)) {
      shared_ptr<TrieNode> node = state.node;
      vector<int> phrase = state.phrase;
      int word_id = edge.second;
      ...
      if (RequiresLookup(node, word_id)) {
        ...
        if (state.starts_with_x) {
          // If the phrase starts with a non terminal, we simply use the
          // matchings from the suffix link.
          ...
          if (next_phrase.Arity() > 0) {
            // For phrases containing a nonterminal, we use either the
            // occurrences of the prefix or the suffix to determine the
            ...
            Clock::time_point intersect_end = Clock::now();
            total_intersect_time += GetDuration(intersect_start, intersect_end);
          } else {
            ...
            phrase_location = matchings_finder->Find(
                ...
                state.phrase.size());
            Clock::time_point lookup_end = Clock::now();
            total_lookup_time += GetDuration(lookup_start, lookup_end);
          }
        ...
        // Automatically adds a trailing non terminal if allowed. Simply copy
        // the matchings from the prefix node.
        ...
        Clock::time_point extract_end = Clock::now();
        total_extract_time += GetDuration(extract_start, extract_end);
      } else {
        next_node = node->GetChild(word_id);
      }

      // Create more states (phrases) to be analyzed.
      vector<State> new_states = ExtendState(lattice, state, edge, phrase,
...
bool GrammarExtractor::CannotHaveMatchings(
...
bool GrammarExtractor::RequiresLookup(
    shared_ptr<TrieNode> node, int word_id) {
  return !node->HasChild(word_id);
}

void GrammarExtractor::AddTrailingNonterminal(
    vector<int> symbols,
    const Phrase& prefix,
...
vector<State> GrammarExtractor::ExtendState(
    const Lattice& lattice,
    const State& state,
  ...
  if (symbols.size() >= max_rule_symbols ||
      state.end + 1 >= lattice.GetSize() ||
      state.phrase_len + 1 >= max_rule_span) {
    return new_states;
  }

  // New state for adding the next symbol.
  int new_phrase_len = state.phrase_len + 1;
  new_states.push_back(State(
      edge.first, new_phrase_len, symbols, state.subpatterns_start,
      node, state.starts_with_x));
  ...
  vector<pair<int, int>> extensions = lattice.GetExtensions(
      edge.first, min_gap_size, max_rule_span - new_phrase_len - 1);
  for (const auto& extension: extensions) {
    subpatterns_start.push_back(extension.first);
    new_states.push_back(State(
        extension.first, new_phrase_len + extension.second, symbols,
        subpatterns_start, node->GetChild(var_id), state.starts_with_x));
